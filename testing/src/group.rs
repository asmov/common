...
use crate::{Module, TestBuilder, Namepath, extension::ExtensionTrait };
...
pub struct Group<'module,'func> {
    ...
    pub(crate) extensions: HashMap<&'static str, Box<dyn ExtensionTrait>>
}
...
impl<'module,'func> Group<'module,'func> {
    ...
    pub fn extension(&self, extension_path: &'static str) -> &Box<dyn ExtensionTrait> {
        if let Some(extension) = self.extensions.get(extension_path) {
            return extension;
        } else {
            panic!("Extension not found: {extension_path}");
        }
    }
...
impl<'module,'func> GroupBuilder<'module,'func> {
    ...
    pub fn extension(mut self, extension: impl ExtensionTrait + 'static) -> Self {
        self.extensions.insert(extension.extension_path_self(), Box::new(extension));
        self
    }
